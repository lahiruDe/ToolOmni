...
import { Search, Menu, Wrench, ChevronRight, X } from 'lucide-react';
...
export function Navbar() {
    ...
    return (
        <nav className="fixed top-0 z-50 w-full border-b border-zinc-100 dark:border-zinc-800 bg-white/70 dark:bg-zinc-950/70 backdrop-blur-xl">
            ...
            {/* Mobile Dropdown */}
            {isMenuOpen && (
                <div className="md:hidden bg-white dark:bg-zinc-950 border-b border-zinc-100 dark:border-zinc-800 animate-in slide-in-from-top duration-200">
                    <div className="px-6 py-8 flex flex-col gap-6">
                        <Link href="/tool/pdf" onClick={() => setIsMenuOpen(false)} className="text-lg font-bold text-zinc-900 dark:text-white flex items-center justify-between">
                            PDF Tools <ChevronRight className="w-5 h-5 text-zinc-300" />
                        </Link>
                        <Link href="/tool/image" onClick={() => setIsMenuOpen(false)} className="text-lg font-bold text-zinc-900 dark:text-white flex items-center justify-between">
                            Image Tools <ChevronRight className="w-5 h-5 text-zinc-300" />
                        </Link>
                        <Link href="/tool/video" onClick={() => setIsMenuOpen(false)} className="text-lg font-bold text-zinc-900 dark:text-white flex items-center justify-between">
                            Video Tools <ChevronRight className="w-5 h-5 text-zinc-300" />
                        </Link>
                        <Link href="/tool/ai" onClick={() => setIsMenuOpen(false)} className="text-lg font-bold text-zinc-900 dark:text-white flex items-center justify-between">
                            AI Tools <ChevronRight className="w-5 h-5 text-zinc-300" />
                        </Link>
                    </div>
                </div>
            )}
